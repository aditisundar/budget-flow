...
class LoanCard(Card):
    ...
    def compute_value(self, current, loans):
        ...
        if self.budgetted == 'minimums':
            ...
            valid, invalid = sorted(list(filter(lambda x: x['ir'] >= 0.1, loans)), key=lambda y: y['ir']), list(
                filter(lambda x: x['ir'] < 0.1, loans))
            self.is_fulfilled = True
            ...
            valid, invalid = sorted(list(filter(lambda x: 0.04 <= x['ir'] < 0.1, loans)), key=lambda y: y['ir']), list(
                filter(lambda x: x['ir'] < 0.04 or x['ir'] >= 0.1, loans))
            self.is_fulfilled = True
            for i in range(len(valid)):
                if current >= valid[i]['loan_remaining_amount']:
                    current -= valid[i]['loan_remaining_amount']
...
class FlowChart:
    ...
    def get_nessie_data(self):
        ...
        self.loans = [{'loan_remaining_amount': 20,
    ...
    def load(self, card_details, default_value):
        ...
        if card_details['category'] == 'emergency':
            ...
            self.current, self.savings = card.compute_value(
                self.current, self.savings)
            card.remaining = self.current
            card.bvalue = card.budgetted
        elif card_details['category'] == 'debts':
            card = LoanCard(card_details)
            card.bvalue = sum([loan['loan_remaining_amount']
                               for loan in self.loans])
            card.budgetted = default_value
            self.current, self.loans = card.compute_value(
                self.current, self.loans)
            card.remaining = self.current
        else:
